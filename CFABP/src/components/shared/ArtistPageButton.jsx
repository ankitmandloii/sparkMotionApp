...
const ArtistPageButton = ({ selectedStage, onStageSelect }) => {
    return (
        <div className="">
            <div
                className="flex items-center justify-between rounded-3xl p-3 shadow-2xl backdrop-blur-md transition-all duration-300 w-full md:max-w-[550px] max-h-[62px] max-w-[300px] "
                style={{
                    // Custom border and box-shadow for the UI effect
                    border: '2px solid rgba(255, 255, 255, 0.1)',
                    boxShadow: '0 4px 6px -1px rgba(1, 2, 2, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.1), 0 0 40px rgba(0, 0, 0, 0.3) inset',
            ...
            >
                {STAGES.map((stage, index) => (
                    <React.Fragment key={stage.name}>
                        {/* Stage Button */}
                        <button
                            className={`flex items-center space-x-2 rounded-lg transition duration-150 ease-in-out shrink-0 px-3 py-2 cursor-pointer`}
                            style={{
                                color: selectedStage === stage.name ? stage.color : '#ffffff',
                                // backgroundColor: selectedStage === stage.name ? stage.color + '20' : 'transparent'
                            }} // Apply dynamic color only on active
